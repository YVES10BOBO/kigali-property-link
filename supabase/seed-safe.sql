-- Safe Seed Data - Prevents Duplicates
-- This version checks if properties already exist before inserting
-- You can run this multiple times without creating duplicates

-- Insert sample properties only if they don't already exist
...
) AS new_properties(title, description, price, price_type, location, bedrooms, bathrooms, area, furnished, parking, security, generator, amenities, status, images)
WHERE NOT EXISTS (
  SELECT 1 FROM properties p
  WHERE p.title = new_properties.title
    AND p.location = new_properties.location
    AND p.price = new_properties.price
    AND p.price_type = new_properties.price_type
)
RETURNING id, title, price, price_type;

-- Verify you have exactly 6 properties
SELECT COUNT(*) as total_properties FROM properties;

-- Show all properties (should be 6)
